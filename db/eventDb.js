...
const createEventDb = async ({
  ...
  const { rows: eventDetails } = await pool.query(
    "INSERT INTO event(template_id, event_date, event_name, venue_name, venue_address, user_id, event_path, used_fonts) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) returning event_id; ",
    [
      templateId,
      eventDate,
      eventName,
...
const createEventCustomDataDb = async (customValuesArray) => {
  const { rows: eventCustomData } = await pool.query(
    format(
      "INSERT INTO custom_data(item_name, item_styles, is_editable, item_value, item_type, event_id, event_path) VALUES %L",
      customValuesArray
    )
  );
